...
const REPOS_STALE_TIME = 10 * 60 * 1000; // 10 min stale
...
export const githubReposInfiniteQuery = infiniteQueryOptions({
    ...
    // Keep checking while any repository is syncing.
    refetchInterval: (query) => {
        const pages = query.state.data?.pages ?? [];

        const isAnyRepoSyncing = pages.some((page) =>
            page.repos.some(
                (repo) =>
        ...
        return isAnyRepoSyncing ? 2000 : false;
    },
});